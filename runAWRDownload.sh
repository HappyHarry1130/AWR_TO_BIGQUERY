...
echo "$PROJECTS" | jq -r '.[]' | while read -r PROJECT; do
  ...
  node ./dist/main.js "$PROJECT" >> ./reports/$LOG_FILE_NAME-DATETIME.log 2>&1
  echo "$PROJECT" >> ./reports/$LOG_FILE_NAME-DATETIME.log 2>&1
...
gsutil cp ./reports/$LOG_FILE_NAME "$DL_GS_BUCKET_NAME"/reports/${LOG_FILE_NAME}.log
